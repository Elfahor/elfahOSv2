...
impl MultibootInfo {
	...
	/// This method returns whether or not `self` has an end tag, and is used to check whether or not this is actually an MBI.
	...
	pub fn get_elf_sections(&self) -> Option<&MemoryMap> {
...
pub unsafe fn load(addr: usize) -> Result<&'static MultibootInfo, MbiErr> {
	// error is null or not 8bit aligned
